...
class MessageDB:
    ...
    def _init_db(self):
        ...
        """)
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_global_user ON user_bindings (global_user_id)")
        conn.commit()

    def create_binding_code(self, code: str, instance_id: str, platform_user_id: str, ttl: int = 300):
        """Create a temporary binding code."""
        conn = self._get_conn()
        cursor = conn.cursor()
        import time
        expires_at = int(time.time()) + ttl
        cursor.execute("INSERT OR REPLACE INTO binding_codes (code, instance_id, platform_user_id, expires_at) VALUES (?, ?, ?, ?)",
                       (code, instance_id, platform_user_id, expires_at))
        conn.commit()

    def consume_binding_code(self, code: str, target_instance: str, target_user_id: str) -> bool:
        ...
        # Find valid code
        cursor.execute("SELECT instance_id, platform_user_id FROM binding_codes WHERE code = ? AND expires_at > ?", (code, now))
        row = cursor.fetchone()
        if not row:
            return False
        ...
        cursor.execute("SELECT global_user_id FROM user_bindings WHERE (instance_id = ? AND platform_user_id = ?) OR (instance_id = ? AND platform_user_id = ?)",
        ...
        # Save bindings
        cursor.execute("INSERT OR REPLACE INTO user_bindings (global_user_id, instance_id, platform_user_id) VALUES (?, ?, ?)",
    ...
    def get_bound_user_id(self, source_instance: str, source_user_id: str, target_instance: str) -> str | None:
    ...
    def remove_user_binding(self, instance_id: str, platform_user_id: str, target_instance_id: str | None = None) -> bool:
        """Remove bindings. If target_instance_id is set, only remove that one. Otherwise remove all."""
        conn = self._get_conn()
        cursor = conn.cursor()
        cursor.execute("SELECT global_user_id FROM user_bindings WHERE instance_id = ? AND platform_user_id = ?",
        ...
        if target_instance_id:
            cursor.execute("DELETE FROM user_bindings WHERE global_user_id = ? AND instance_id = ?", 
                           (global_id, target_instance_id))
        else:
            cursor.execute("DELETE FROM user_bindings WHERE global_user_id = ?", (global_id,))
        conn.commit()
        return True

    def get_all_bindings(self, instance_id: str, platform_user_id: str) -> list[tuple[str, str]]:
        ...
        global_id = row[0]
        cursor.execute("SELECT instance_id, platform_user_id FROM user_bindings WHERE global_user_id = ?", (global_id,))
    ...
    def save_user(self, instance_id: str, platform_user_id: str, display_name: str):
        ...
        try:
            ...
            l.error(f"Failed to save user mapping: {e}")
    ...
    def get_mapped_user_id(self, source_instance: str, source_user_id: str, target_instance: str) -> str | None:
        """Find the target user ID mapped from a source user ID via display name."""
        conn = self._get_conn()
        cursor = conn.cursor()
        # 1. Get source display name
        cursor.execute("SELECT display_name FROM user_mappings WHERE instance_id = ? AND platform_user_id = ?",
