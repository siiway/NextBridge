...
| `forward_render_ttl_seconds` | 否 | `15552000`（180 天） | 合并转发 HTML 页面有效期（秒）。页面会在到期后于当前页直接切换为“已销毁”。最小值为 60 秒。 |
...
| `forward_render_image_method` | 否 | `"url"` | 合并转发页面中图片渲染方式。`"url"`：图片二进制写入数据库并通过桥接服务资源 URL 提供；`"base64"`：直接把 data URI 内嵌进页面。 |
| `forward_render_base_url` | 否 | `""` | 合并转发链接优先使用的公开前缀。设置后将按 `${forward_render_base_url}/{page_id}` 生成，**不会**自动拼接 `forward_render_mount_path`。适合路径反代场景。 |
| `forward_render_asset_ttl_seconds` | 否 | `1209600`（14 天） | 合并转发图片资源在桥接服务上保留的有效期（秒）。设置为 `0` 表示永久有效。 |
...
| `file` | 作为 `file` 附件转发；在合并转发中：显示名称/大小/file_id 元信息并尽量解析下载链接 |
| `forward`（合并转发） | 调用 `get_forward_msg` 拉取节点，渲染为临时 HTML 页面并转发链接；嵌套合并转发会递归渲染；语音嵌入为可播放音频；图片/表情按 `forward_render_image_method` 渲染；文件显示元信息 |
...
如果你想手动切换到中国大陆加速源，可以把 `forward_render_cqface_gif` 设为字符串，例如：
