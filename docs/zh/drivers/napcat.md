...
| `forward_render_base_url` | 否 | `""` | 合并转发链接优先使用的公开前缀。设置后将按 `${forward_render_base_url}/{page_id}?t=...` 生成，**不会**自动拼接 `forward_render_mount_path`。适合路径反代场景。 |
| `forward_render_asset_ttl_seconds` | 否 | `86400` | 合并转发图片资源在桥接服务上保留的有效期（秒）。设置为 `0` 表示永久有效。 |
...
| `text` | 转为消息文本 |
| `at` | 转为 `@名称` 格式的文本 |
| `image` | 作为 `image` 附件转发；在合并转发中：下载并嵌入为 data URI（如果超大则不处理），无法获取时显示占位符和原始链接 |
...
| `forward`（合并转发） | 调用 `get_forward_msg` 拉取节点，渲染为临时 HTML 页面并转发链接；嵌套合并转发会递归渲染；语音嵌入为可播放音频；图片/表情下载并嵌入；文件显示元信息 |
