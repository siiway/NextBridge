...
说明：`接收` 表示平台消息能否进入 NextBridge；`发送` 表示 NextBridge 能否向该平台发消息。

| 平台                     | 状态  | 接收  | 发送  | 提及 (Mention) | 回复 (Reply) | 媒体文件 | 自定义代理 | 备注                                                 |
| :--------------------- | :-- | :-- | :-- | :----------- | :--------- | :--- | :---- | :------------------------------------------------- |
| **腾讯 QQ**（QQ 驱动器）   | ✅   | ✅   | ✅   | ✅            | ✅          | ✅    | ✅     | 支持 NapCat、Lagrange 和通用 OneBot 11 协议                        |
...
| **云湖**                 | ✅   | ✅   | ✅   | ❌            | ✅         | ℹ️    | ✅     | Webhook 接收；开放 API 发送；**文件接收不可用，文件发送可用**；**暂未支持原生提及语法** |
